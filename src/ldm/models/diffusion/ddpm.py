...
class ImageEmbeddingConditionedLatentDiffusion(LatentDiffusion):
    def __init__(self, embedder_config, depth_stage_config=None, embedding_key="jpg", embedding_dropout=0.5,
...
class ImageEmbeddingConditionedInpaintingLatentDiffusion(LatentFinetuneDiffusion):
    def __init__(self, embedder_config, embedding_key="jpg", embedding_dropout=0.5,
                 concat_keys=("mask", "masked_image"), depth_stage_config=None,
                 masked_image_key="masked_image",
                 freeze_embedder=True, noise_aug_config=None, *args, **kwargs):
        super().__init__(concat_keys, *args, **kwargs)
        self.masked_image_key = masked_image_key
    ...
    def get_input(self, batch, k, cond_key=None, bs=None, return_first_stage_outputs=False, **kwargs):
        # note: restricted to non-trainable encoders currently
        assert not self.cond_stage_trainable, 'trainable cond stages not yet supported for inpainting'
        z, c, x, xrec, xc = super().get_input(batch, self.first_stage_key, return_first_stage_outputs=True,
                                              force_c_encode=True, return_original_cond=True, bs=bs)
        ...
        if self.training:
            c_adm = torch.bernoulli((1. - self.embedding_dropout) * torch.ones(c_adm.shape[0],
                                                                               device=c_adm.device)[:, None]) * c_adm
        all_conds = {"c_concat": [c_cat], "c_crossattn": [c], 'c_adm': c_adm}
        # noutputs = [z, all_conds]
        # noutputs.extend(outputs[2:])
        # return noutputs
        return z, all_conds
