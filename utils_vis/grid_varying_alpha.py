...
def create_grids(base_dir, output_dir):
    ...
    for index in range(11):
        ...
        for reps in range(0, 8):
            for alpha_dir in sorted(glob.glob(os.path.join(base_dir, 'cond_alpha_*')), key=lambda x: float(x[x.rfind('_') + 1:])):
                f = glob.glob(os.path.join(alpha_dir, f'index_{index}', 'samples/', f'*_{reps}.png'))
...
if __name__ == "__main__":
    base_dir = 'output/cond_alpha/'
    output_dir = 'output/cond_alpha/results/'
